...
output_dir=${WILDLIFE_LOMA_OUTPUT:-${CHECKPOINTS_ROOT}/wildlife-reid-10k/${WILDLIFE_DATASET_ID}/loma-finetuned/${protocol}}
...
[[ -f "${cache_dir}/manifest.json" ]] || bash "${LYNX_FINETUNING_ROOT}/slurm_scripts/build_wildlife_loma_cache.sh"
...
accelerate launch --num_processes "${WILDLIFE_NUM_PROCESSES:-4}" --num_machines 1 \
  ...
  --eval_every_epochs "${WILDLIFE_EVAL_EVERY:-10}" --seed 0 --resize 512 --num_keypoints 512 "${resume_args[@]}"
}
