...
def test_descriptor_plus_matcher_mode_unfreezes_both_but_never_the_detector():
    model = FakeDescriptorLoMa()
    configure_loma_trainable_component(model, "descriptor+matcher")
    set_loma_train_mode(model, True, "descriptor+matcher")

    assert all(parameter.requires_grad for parameter in model._descriptor.parameters())
    assert all(parameter.requires_grad for parameter in model.input_proj.parameters())
    assert all(parameter.requires_grad for parameter in model.transformers.parameters())
    assert all(not parameter.requires_grad for parameter in model._detector.parameters())
    assert not model._detector.training
    assert model._descriptor.training and model.input_proj.training
    set_loma_train_mode(model, False, "descriptor+matcher")
    assert not model._descriptor.training and not model.input_proj.training
    with pytest.raises(ValueError):
        configure_loma_trainable_component(model, "all")


def test_descriptor_plus_matcher_forward_reaches_descriptors_and_matcher_weights():
    model = FakeDescriptorLoMa()
    configure_loma_trainable_component(model, "descriptor+matcher")
    ...
    assert any(
        ...
        for parameter in model.input_proj.parameters()
    )
